...
/*
   ...
   Задача: 
            ...
            одномерный массив индексов A. 
            В А хранятся индексы столбцов матрицы в отсортированном виде, 
            в порядке возрастания сумм элементов столбцов. 
            В основной программе сформировать матрицу, получить индексный массив, 
            использовать его для вывода столбцов матрицы в порядке возрастания сумм их элементов.
*/

namespace Task03Page32
{
    class Program
    {
        ...
        static int CompareSumCols(int a, int b)
        {
            return (SumCol(matrix, a) >= SumCol(matrix, b)) ? 1 : -1;
        }
        ...
        static void Main()
        {
            do
            {
                ...
                Array.Sort(a, CompareSumCols);
                Console.WriteLine("Matrix sorted by sum of columns:");
                for (int i = 0; i < n; ++i)
                {
                    for (int j = 0; j < m; ++j)
                    {
                        Console.Write(matrix[i, a[j]] + " ");
                    }
                    Console.WriteLine();
                }
