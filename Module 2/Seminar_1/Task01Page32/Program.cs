...
namespace Task01Page32
{
    class Program
    {
        ...
        /// <summary>
        /// Summarizes the elements of row K of matrix.
        /// </summary>
        /// <returns>Sum.</returns>
        /// <param name="matrix">Matrix.</param>
        /// <param name="k">Number of row.</param>
        static int SumRow(int[,] matrix, int k)
        {
            int sum = 0;
        ...
        /// <summary>
        /// Multiplies the elements of row K of matrix.
        /// </summary>
        /// <returns>Product.</returns>
        /// <param name="matrix">Matrix.</param>
        /// <param name="k">Number of row.</param>
        static int MultRow(int[,] matrix, int k)
        {
            int pr = 1;
            for (int i = 0; i < matrix.GetLength(1); ++i)
                pr *= matrix[k, i];
        ...
        static void Main()
        {
            do
            {
                ...
                int n = InputVar("number of columns in matrix", 1, int.MaxValue, (x, y) => x < y, (x, y) => x > y);
                int m = InputVar("number of rows in matrix", 1, int.MaxValue, (x, y) => x < y, (x, y) => x > y);

                int[,] matrix = CreateMatrix(n, m);

                Console.WriteLine($"Matrix:\n{MatrixToString(matrix)}");

                int k = InputVar($"number of row (1 - {n})", 1, n, (x, y) => x < y, (x, y) => x > y);
                k--;

                Console.WriteLine($"Sum of row: {SumRow(matrix, k)}, Product of row: {MultRow(matrix, k)}");
