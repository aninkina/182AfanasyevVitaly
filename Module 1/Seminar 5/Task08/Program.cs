...
/*
   ...
   Задача: 
            Получить от пользователя число N - размер массива. Сформировать массив по формуле:
                a[i] = (i * (i + 1) / 2) % N.
            Нормировать массив (разделить каждый элемент на максимальный).
            Вывести исходный и нормированный массивы.
*/
...
namespace Task08
{
    class Program
    {
        ...
        /// <summary>
        /// Forms the array with formula: a[i] = (i * (i + 1) / 2) % N.
        /// </summary>
        /// <returns>Array.</returns>
        /// <param name="n">N.</param>
        static double[] FormArray(int n)
        {
        ...
        /// </summary>
        /// <returns><c>true</c>, if array was normalized, <c>false</c> otherwise.</returns>
        /// <param name="array">Array.</param>
        static bool NormalizeArray(double[] array)
        {
            double max = -1;
        ...
        static void Main()
        {
            do
            {
                ...
                if (NormalizeArray(a))
                {
                    Console.Write("Normalized array: ");
                    OutputArray(a);
                }
                else
                    Console.WriteLine("Array can't be normalized!");  
